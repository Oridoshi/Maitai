...
export default function Ticket()
{
	...
	function editTickets(ligne)
	{
		...
		if(ligneExist){
			...
			//creation de la ligne avec un id edit pour la trouver et la masquer
			const ligneEdit = document.createElement('tr');
			ligneEdit.id = 'edit';
			ligneEdit.classList.add("col-sm-12");

			initialHeader.forEach(column => {
				const col = document.createElement('td');
				col.textContent = ''; // Contenu vide par défaut
				ligneEdit.appendChild(col);
			});

			//cherche le client dont on veut edit le ticket et ajoute la ligneEdit en dessous
