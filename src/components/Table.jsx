    ...
    const handleSubmit = async (e) => {
        ...
        if (id !== '') {
            ...
            if (insertSuccess) {
                ...
                openModal(updatedRowData);
            }
        }
    };

    const changeThing = (item, columnId) => {
                ...
                <table className='tableau table table-hover my-0'>
                    ...
                    <tbody>
                        {datas.slice().reverse().map(item => (
                            <tr className={`bg-light ${typeof keyGrayWhenFalse === 'string' && (item[keyGrayWhenFalse] === false || item[keyGrayWhenFalse] === 0) === false ? '' : 'text-muted'}`} key={`${item.id}`}>
                                {header.map(column => (
                                    column.show && (
